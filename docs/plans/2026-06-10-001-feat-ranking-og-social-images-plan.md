...
- AE1. Given a user publishes "Best public goods projects" with five ranked entries, when publish completes, the app generates landscape and story PNGs, uploads them to `RANKING_OG_PUBLIC_BASE_URL/og/rankings/{rankEntityId}/{version}/landscape.png` and `RANKING_OG_PUBLIC_BASE_URL/og/rankings/{rankEntityId}/{version}/story.png`, and navigates to a share URL containing `rankEntityId`, `authorSpaceId`, and `ogVersion`.
...
- Required production storage should use a Cloudflare custom domain, for example `https://img.geobrowser.io`, configured as `RANKING_OG_PUBLIC_BASE_URL`.
...
- Suggested env names:
  ...
  - `RANKING_OG_PUBLIC_BASE_URL`
  - `RANKING_OG_ADMIN_SECRET` for script or admin-triggered backfills only
