...
export async function generate({ paths, inputFs = require('fs'), outputFs = require('fs') }: Configuration) {
	...
	const generatedTypes = getTypeNames(schema).map((name) => [`/${name}.ts`, generateType(schema, name)])
...
async function generateIndexFile(fs: IFS, directoryPath: string) {
	...
	const source = files.map((name) => `export * from "./${path.basename(name, '.ts')}"`).join('\n')
