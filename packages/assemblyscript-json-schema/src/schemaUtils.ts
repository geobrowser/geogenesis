...
type JSONValue = null | boolean | number | string | { [x: string]: JSONValue } | Array<JSONValue>
...
export function getResolvedReference<T = unknown>(schema: JSONSchema7, ref: string): T {
	if (ref.startsWith('#')) {
...
export function getAnyOfTypeNames(definition: JSONSchema7) {
	const anyOf = (definition.anyOf || []).flatMap((schema) => (typeof schema === 'boolean' ? [] : [schema]))
...
export function hasArrayOfComplexTypes(schema: JSONSchema7) {
	return !!traverse.find(
		['', schema as JSONValue],
		([key, value]) => key === 'items' && typeof value === 'object' && value != null && '$ref' in value
	)
}
