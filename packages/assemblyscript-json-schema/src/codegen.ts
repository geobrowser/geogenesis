...
export function generateUnionType(schema: JSONSchema7, name: string, definition: JSONSchema7) {
  ...
  export class ${name} {
    ...
    ${anyOfTypeNames
            ...
            return this.type == '${getDiscriminator(getDefinition(schema, typeName))}' ? (this as ${typeName}) : null;
          }`
			)
			.join('\n\n')}
...
export function generateObjectType(schema: JSONSchema7, name: string, definition: JSONSchema7) {
	const { properties = {} } = definition
	const unions = getUnionsContainingType(schema, name)
	const importedNames = unique([...findRefs(definition).map(getRefName), ...unions])
  ...
  ${hasArrayOfComplexTypes(definition) ? `import { mapOrNull } from "./collection-utils"` : ''}  

  export class ${name} ${unions.length > 0 ? `extends ${unions.join(', ')}` : ''} {
    ${standardProperties
			.map(([property, value]) => `${property}: ${convertTypeName(schema, value as JSONSchema7)}`)
			.join('\n')}
      
    constructor(${standardProperties
			.map(([property, value]) => `${property}: ${convertTypeName(schema, value as JSONSchema7)}`)
			.join(', ')}) {
      ${unions.length > 0 ? `super(${JSON.stringify(getDiscriminator(definition))})` : ''}
      ${standardProperties.map(([property]) => `this.${property} = ${property}`).join('\n')}
    }
      ...
      ${Object.entries(properties)
            ...
            for (let i = 0; i < this.${property}.length; i++) {
              ${property}.push(this.${property}[i].toJSON())
						...
						result += `const ${property} = ${getEncoderFunction(value as JSONSchema7)}(this.${property})`
					}
    ...
    static fromJSON(__json: JSON.Value): ${name} | null {
      ...
      ${Object.entries(properties)
				.map(([property, value]) => {
					...
					let result = `
          let __${property} = ${getDecoderFunction(property, value as JSONSchema7)}
          ${
