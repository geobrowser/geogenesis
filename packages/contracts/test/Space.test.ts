...
import { deploySpaceBeacon, deploySpaceInstance, upgradeToSpaceV2 } from '../src/deploy'
...
describe('Space', () => {
	...
	it('Grants and revokes role', async () => {
		...
		expect(await contract.hasRole(await contract.EDITOR_ROLE(), address1.address)).to.be.eq(false)

		await contract.grantRole(await contract.EDITOR_ROLE(), address1.address)

		expect(await contract.hasRole(await contract.EDITOR_ROLE(), address1.address)).to.be.eq(true)
