...
export class ImageValue extends Value {
  ...
  static fromJSON(__json: JSON.Value): ImageValue | null {
    if (!__json.isObj) {
      log.debug('ImageValue.fromJSON(): __json.isObj is false', [])
      return null
    }
    ...
    if (__type == null) {
      log.debug('ImageValue.fromJSON(): __type is null', [])
      return null
    }
    ...
    if (__id == null) {
      log.debug('ImageValue.fromJSON(): __id is null', [])
      return null
    }
    ...
    if (__value == null) {
      log.debug('ImageValue.fromJSON(): __value is null', [])
      return null
    }
    const value = __value.valueOf()

    return new ImageValue(id, value)
  }
}
