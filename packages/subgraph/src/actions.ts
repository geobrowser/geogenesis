import { Action, CreateEntityAction, CreateTripleAction, DeleteTripleAction } from '@geogenesis/action-schema/assembly'
...
export function handleCreateTripleAction(options: HandleCreateTripleActionOptions): void {
	const fact = options.fact
	const space = options.space
	const isProtected = options.isProtected
	const createdAtBlock = options.createdAtBlock

	const entity = (GeoEntity.load(fact.entityId) || new GeoEntity(fact.entityId))!
	entity.save()

	const attribute = (GeoEntity.load(fact.attributeId) || new GeoEntity(fact.attributeId))!
	attribute.save()

	const tripleId = createTripleId(space, fact.entityId, fact.attributeId, fact.value)
	...
	if (existing && existing.isProtected) {
		log.debug(`Couldn't create or update triple '${tripleId}' since it already exists and is protected!`, [])
		return
	}
	...
	if (stringValue) {
		...
		log.debug(`space: ${space}, entityId: ${entity.id}, attributeId: ${attribute.id}, value: ${stringValue.value}`, [])
...
function handleDeleteTripleAction(fact: DeleteTripleAction, space: string): void {
	const tripleId = createTripleId(space, fact.entityId, fact.attributeId, fact.value)
...
function handleCreateEntityAction(action: CreateEntityAction): void {
	const entity = (GeoEntity.load(action.entityId) || new GeoEntity(action.entityId))!
...
export function handleAction(action: Action, space: string, createdAtBlock: BigInt): void {
