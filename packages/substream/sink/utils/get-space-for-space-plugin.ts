 ...
 *
 * This function looks up the space-id for a given space plugin address address.
 *
 ...
 */
export function getSpaceForSpacePlugin(
  ...
  return Effect.gen(function* (unwrap) {
    const spaceExistsForPluginAddress = yield* unwrap(
      Effect.tryPromise({
        try: () =>
          db
            .selectOne('spaces', { space_plugin_address: getChecksumAddress(pluginAddress) }, { columns: ['id'] })
            .run(pool),
