...
import { mapMembers } from './map-editors';
...
export function handleMembersAdded(editorsAdded: EditorAdded[], block: BlockEvent) {
  return Effect.gen(function* (_) {
    const telemetry = yield* _(Telemetry);
    const schemaMembers = yield* _(mapMembers(editorsAdded, block));
