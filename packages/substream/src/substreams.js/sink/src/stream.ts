...
export function createStream({
    ...
    const create = Effect.gen(function* (_) {
        ...
        const currentCursor = yield* _(
            Ref.make(startCursor ? Option.some<string>(startCursor) : Option.none<string>())
        );
            ...
            Effect.gen(function* (_) {
                ...
                switch (kind) {
                            ...
                            for (const [j, range] of stage.completedRanges.entries()) {
                                ...
                                if (productionMode && i === message.stages.length - 1) {
                                    if (startBlock <= range.startBlock && range.endBlock >= startBlock) {
                                        const metric = Metric.tagged(
                                            Metrics.ProgressMessageLastContiguousBlock,
                                            'stage',
                                            `stage-${i}`
                                        );
                    ...
                    case 'debugSnapshotData': {
                        yield* _(
                            Effect.logWarning(
                                'Received debug snapshot message, there is no reason to receive those here'
                            )
                        );
                        return;
                    }
        ...
        const aquire = Ref.get(currentCursor).pipe(
            ...
            Effect.map(startCursor => {
                ...
                const stream = Stream.fromAsyncIterable(
                    client.blocks(request, { signal: controller.signal }),
                    cause => {
        ...
        const stream = Stream.acquireRelease(aquire, (scope, exit) =>
            Effect.sync(() => scope.controller.abort(exit))
        ).pipe(
