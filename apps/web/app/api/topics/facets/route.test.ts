...
describe('POST /api/topics/facets', () => {
  ...
  it('bounds selected Topics before building relation-heavy facet filters', async () => {
    ...
    expect(response.status).toBe(200);
    expect(mocks.fetchFacets.mock.calls[0]?.[0].selectedTopicIds).toEqual(selectedTopicIds.slice(0, 10));
  });
});
