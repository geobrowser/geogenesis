...
- **Be economical.** Prefer one well-scoped query over several. You have about 25 seconds of querying before your tool is taken away and you are made to answer, and a human is waiting on a spinner. A round trip costs 5 seconds at best and a broad one 15, so budget for two or three queries, not eight. \`first: 0\` with \`totalCount\` is the cheapest possible query when only a count is needed.
