...
import { topicExploreRowsDocument } from './use-topic-explore-rows';
...
function coverageEntityFields(): string[] {
  const root = fields(operation(topicExploreRowsDocument).selectionSet.selections)[0];
...
describe('the topic coverage document', () => {
  ...
  it('scopes the relation, both its ends, and nothing else', () => {
    ...
    const printed = print(topicExploreRowsDocument).replace(/\s+/g, ' ');
  ...
  it('does not scope its value and relation lists to a space list it cannot have', () => {
    ...
    expect(print(topicExploreRowsDocument)).not.toContain('spaceIdsForLists');
    expect((operation(topicExploreRowsDocument).variableDefinitions ?? []).map(v => v.variable.name.value)).not.toContain(
      'spaceIdsForLists'
    );
  });
