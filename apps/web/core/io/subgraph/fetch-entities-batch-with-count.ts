...
export async function fetchEntitiesBatchCached(options: FetchEntitiesBatchCachedOptions) {
  ...
  return queryClient.fetchQuery({
    ...
    queryFn: () => fetchEntitiesBatchWithCount(options),
  });
}
...
interface NetworkResult {
  entities: { nodes: SubstreamEntityLive[]; totalCount: number };
}
...
export async function fetchEntitiesBatchWithCount(
  options: FetchEntitiesBatchOptions
    ...
    if (Either.isLeft(resultOrError)) {
      ...
      switch (error._tag) {
        ...
        case 'GraphqlRuntimeError':
          ...
          return { entities: { nodes: [], totalCount: 0 } };
  ...
  const unknownEntitiesGrouped = unknownEntities.entities.nodes
    .map(e => {
  ...
  return { entities: unknownEntitiesGrouped, totalCount: unknownEntities.entities.totalCount };
}
