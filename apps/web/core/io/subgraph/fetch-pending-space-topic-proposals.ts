...
const SPACE_TOPIC_ACTION_TYPES = ['TopicDeclared', 'TopicRemoved'];
...
async function fetchTopicMetadata(topicIds: string[]) {
  if (topicIds.length === 0) {
    return new Map<
      string,
      { name: string | null; description: string | null; image: string; spaces: TopicUsage['spaces']; spacesCount: number }
    >();
  }
