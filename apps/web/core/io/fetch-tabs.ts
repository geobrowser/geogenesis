...
export async function fetchTabs(ids: string[]): Promise<Entity[]> {
  const queryId = v4();
    ...
    if (Either.isLeft(resultOrError)) {
      ...
      switch (error._tag) {
        ...
        case 'GraphqlRuntimeError':
          console.error(
            `Encountered runtime graphql error in fetchTabs. queryId: ${queryId} endpoint: ${endpoint} ids: ${ids}
        ...
        default:
          console.error(
            `${error._tag}: Unable to fetch fetch tabs, queryId: ${queryId} endpoint: ${endpoint} ids: ${ids}`
          );
          return {
            entities: { nodes: [] },
          };
