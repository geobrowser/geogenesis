 ...
 *
 * Accepting a request creates the debate and pushes straight into its room, but the room is a
 * server segment with no `loading` boundary: the router holds the current page on screen until the
 * payload arrives — seconds, on a cold route. The same `onSuccess` invalidates activity, which
 * comes back long before that. For those seconds the accepting tab is a tab with an active debate
 * sitting on some other path, which is exactly the shape `DebateCoordinator` reads as "this person
 * has not been told their debate is ready" — so it reopened the very dialog they had just accepted
 * from, with the buttons swapped, and took it away again when the room finally landed. That is the
 * flicker.
 *
 * Recording the intent here closes the gap: a debate this tab is walking into counts as one it is
 * already in, for as long as the walk takes.
 */
