...
export function useDebatePlayback(debate: Debate, enabled: boolean) {
  ...
  /**
   * Every turn's place on the timeline, for the overlays that have to look past the current one.
   *
   * `turnStateAt` answers "who is speaking and how long have they got", which is what the audible
   * panel and the ring need. A cue that fires *before* a turn — the up-next chip, the hand-off —
   * has to know the turn after this one, so it needs the list rather than the answer. Same
   * precedence as `turnStateAt`: the rendered segments where they exist, the format's allowance
   * where they do not.
   */
