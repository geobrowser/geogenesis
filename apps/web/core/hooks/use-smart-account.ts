...
export function useSmartAccount() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // (The legacy v1 Safe+Pimlico mainnet stack was removed with the v2
      // registry: it keyed permissions on the Safe address, while the v2
      // registry keys on the Privy EOA.) Use the Privy embedded wallet as the
      // EOA signer — toViemAccount wraps it in a viem LocalAccount whose
      // signAuthorization routes through Privy's signing API (the wagmi
      ...
      // GEO_NETWORK carries the chain, RPC, and sponsorship endpoints for the
      // configured network.
