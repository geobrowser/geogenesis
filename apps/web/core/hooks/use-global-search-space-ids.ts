'use client';

import * as React from 'react';

import { useQuery, useQueryClient } from '@tanstack/react-query';

import { browseSidebarDataQueryKey } from '~/core/browse/browse-sidebar-query';
import type { BrowseSidebarData } from '~/core/browse/fetch-browse-sidebar-data';
import { ROOT_SPACE } from '~/core/constants';

import { usePersonalSpaceId } from './use-personal-space-id';
import { useSpaceId } from './use-space-id';

/**
 * Space ids that are always passed as `additional_space_ids` to the REST
 * /search endpoint: root, the user's current space (if any), and the user's
 * personal space (if any), plus accepted editor/member spaces already loaded
 * by the browse sidebar. Use this anywhere the REST search api is called from a
 * client component / hook so results from these spaces surface regardless of
 * the active scope.
 */
export function useGlobalSearchSpaceIds(): string[] {
  const queryClient = useQueryClient();
  const currentSpaceId = useSpaceId();
  const { personalSpaceId } = usePersonalSpaceId();
  const sidebarQueryKey = browseSidebarDataQueryKey(personalSpaceId);
  const { data: browseSidebarData = null } = useQuery<BrowseSidebarData | null>({
    queryKey: sidebarQueryKey,
    enabled: false,
    initialData: () => queryClient.getQueryData<BrowseSidebarData>(sidebarQueryKey) ?? null,
  });

  const memberAndEditorSpaceIds = React.useMemo(() => {
    if (!browseSidebarData) return [];

    return [...browseSidebarData.editorOf, ...browseSidebarData.memberOf]
      .filter(row => !row.pendingLabel)
      .map(row => row.id);
  }, [browseSidebarData]);

  return React.useMemo(
    () =>
      Array.from(
        new Set(
          [ROOT_SPACE, currentSpaceId, personalSpaceId, ...memberAndEditorSpaceIds].filter(
            (id): id is string => Boolean(id)
          )
        )
      ),
    [currentSpaceId, memberAndEditorSpaceIds, personalSpaceId]
  );
}
