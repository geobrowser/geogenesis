...
describe('pendingEntityMatchesRanking', () => {
  ...
  it('with no relation constraints, accepts a newly-named entity', () => {
    const diff = makeDiff({
      relations: [],
      values: [
        { propertyId: SystemIds.NAME_PROPERTY, spaceId: 'space-a', type: 'TEXT', before: null, after: 'New', diff: [] },
      ],
    });
  ...
  it('with no relation constraints, rejects an entity with no added content', () => {
    const diff = makeDiff({
      relations: [],
      values: [
        { propertyId: SystemIds.NAME_PROPERTY, spaceId: 'space-a', type: 'TEXT', before: 'Old', after: 'New', diff: [] },
      ],
    });
