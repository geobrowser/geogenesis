...
export function buildBounty(
  ...
  personalSubmissionCounts: Map<string, number>,
  spaceId?: string,
  // Unused, but every caller passes it positionally; removing it breaks them.
  ...
  const submissionsPerPerson = parseNumber(
    findValueById(entityValues, BOUNTY_MAX_SUBMISSIONS_PER_PERSON_PROPERTY_ID) ??
      findFirstRelationName(entityRelations, BOUNTY_MAX_SUBMISSIONS_PER_PERSON_PROPERTY_ID)
  );
...
export function hasBountyTaskStatusDoneRelation(relations: StoreRelation[]): boolean {
  const doneHex = uuidToHex(BOUNTY_STATUS_DONE_ID);
  return relations.some(relation => {
