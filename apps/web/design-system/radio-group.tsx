  ...
  <Component.Root value={value} onValueChange={onValueChange} className={cx('flex flex-col gap-3')} {...rest}>
    {options.map(({ label, image, value, disabled = false, ...rest }: Option) => (
      <Component.Item
        ...
        value={value}
        className={cx(
          'flex w-full items-center justify-between rounded-lg border border-grey-02 p-1.5 pr-3 text-grey-04 data-[state=checked]:border-text data-[state=checked]:text-text',
      ...
      >
        <div className="inline-flex items-center gap-4">
          {image && (
            <div className="rounded-md relative h-12 w-12 overflow-clip">
