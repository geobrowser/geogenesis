...
export default function PdfZoom({
  ...
  return (
    <div className="flex h-full flex-col">
      <button
        onClick={openPdf}
        className={`cursor-pointer ${className ? className : 'h-[120px] min-h-[120px] w-[173px] overflow-hidden'} ${isEditing ? 'border-b border-b-[#D9D9D9]' : 'rounded-lg'}`}
        title="Open PDF"
      >
        <Document
          file={src}
      ...
      {isEditing && (
        <div className="flex flex-grow flex-col justify-between p-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={openPdf}
              className="text-[11px] font-medium leading-[13px] text-grey-04 hover:text-text"
            >
              Open
            </button>
          </div>
