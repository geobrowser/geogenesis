...
export const TableBlockFilterPrompt = React.forwardRef<TableBlockFilterPromptHandle, TableBlockFilterPromptProps>(
  function TableBlockFilterPrompt(
     ...
     *
     * Reads through `stateRef` rather than the render closure so it is safe to call from
     * a dismiss handler, where the latest toggle may have landed after this render.
     */
    ...
    const onOpenChange = (open: boolean) => {
      ...
      if (!open) {
        ...
        // so anything not applied here would silently disappear. Relations mode keeps
        // its own state in `StaticRelationsFilters` and has no column drafts to apply.
        if (isRelationsMode) {
