  ...
  for (const op of createImageOps) {
    if (op.type === 'createRelation') {
      relations.push({
        ...
        type: { id: toHexId(op.relationType), name: 'Image' },
        toEntity: { id: toHexId(op.to), name: 'Image', value: toHexId(op.to) },
        spaceId,
        position: Position.generate(),
      ...
      for (const pv of op.values) {
        values.push({
          ...
          property: { id: toHexId(pv.property), name: 'Image Property', dataType: 'TEXT', renderableType: 'URL' },
          spaceId,
